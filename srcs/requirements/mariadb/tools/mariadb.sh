...
mysql -u root -p"${SQL_ROOT_PASSWORD}" -e "CREATE DATABASE IF NOT EXIST \`${SQL_DATABASE}\`;" || { echo "Failed to create database"; exit 1; }
mysql -u root -p"${SQL_ROOT_PASSWORD}" -e "CREATE USER IF NOT EXIST '${SQL_USER}'@'localhost' IDENTIFIED BY '${SQL_PASSWORD}';" || { echo "Failed to create user"; exit 1; }
...
mysql -u root -p"${SQL_ROOT_PASSWORD}" -e "DROP DATABASE \`${SQL_DATABASE}\`;" && { echo "Failed to set SQL mode"; exit 1; }
