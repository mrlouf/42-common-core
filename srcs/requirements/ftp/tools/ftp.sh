...
mkdir -p /opt/locale/etc/pure-ftpd/pdb
pure-pw mkdb /opt/locale/etc/pure-ftpd/pdb/pureftpd.ftp
...
# Start Pure-FTPd
exec /usr/sbin/pure-ftpd -S 2121 -l puredb:/opt/local/etc/pure-ftpd/pdb/pureftpd.ftp -E -j -R -P $PASV_ADRESS -p $PASV_MIN_PORT:$PASV_MAX_PORT